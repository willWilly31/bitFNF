  ...
  useEffect(() => {
    ...
    class Particle {
      ...
      draw() {
        ...
        context.fillStyle = "rgba(72, 203, 255, 0.9)";
        context.fill();
      }
    }
    ...
    const resizeCanvas = () => {
      ...
      const count = Math.max(55, Math.floor((clientWidth * clientHeight) / 12000));
      ...
      for (let i = 0; i < particles.length; i += 1) {
        for (let j = i + 1; j < particles.length; j += 1) {
          ...
          if (distance < 180) {
            ...
            context.strokeStyle = `rgba(150, 110, 255, ${opacity * 0.85})`;
    ...
    const animate = () => {
      context.fillStyle = "rgba(5, 7, 18, 0.95)";
    ...
    <div className="min-h-screen bg-gradient-to-b from-background via-muted/30 to-background">
      ...
      <main className="relative">
        ...
        <section id="why" className="container mx-auto px-4 py-20 relative overflow-hidden">
          <div className="absolute inset-x-2 md:inset-x-4 bottom-2 top-28 rounded-[2rem] overflow-hidden">
            <WhySectionParticleBackdrop />
            <div className="absolute inset-0 bg-gradient-to-b from-background/20 via-background/10 to-background/25" />
          </div>
